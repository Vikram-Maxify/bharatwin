...
export const feedback = createAsyncThunk(
  "user/feedback", // Changed to 'user/feedback' for consistency with slice name
  async (info, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.post("/webapi/feedback", info, {
        withCredentials: true,
      });
      return fulfillWithValue(data);
    } catch (error) {
  ...
  extraReducers: (builder) => {
    builder          
      ...
      .addCase(rechargeList2.fulfilled, (state, { payload }) => {
       state.successMessage = payload.message;
       state.rechargeList2Data=payload.data2
        state.loader = false;
      })
      ...
      .addCase(feedback.rejected, (state, { payload }) => {
        state.errorMessage = payload?.message || "An error occurred during feedback submission.";
        state.loader = false;
      })
      .addCase(feedback.fulfilled, (state, { payload }) => {
        state.successMessage = payload.message || "Feedback submitted successfully!";
        state.feedbackData = payload.data; // Store the feedback data if needed
        state.loader = false;
      })
