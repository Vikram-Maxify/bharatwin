  ...
  try {
    ...
    if (user_id.length > 0) {
      ...
      await connection.query(
        `UPDATE users SET recharge = ? WHERE id_user = ?`,
        [money_value, id_user],
      );
      // }
      // if (buff_acc == "2") {
  ...
  try {
    if (type === "confirm") {
      ...
      if (Firstrecharge.length === 0) {
        result.sort(
          (a, b) => parseFloat(a.recAmount) - parseFloat(b.recAmount),
        );
        ...
        if (refferaluser.length > 0) {
          await connection.query(
            "UPDATE users SET money = money + ?, total_money = total_money + ?, recharge = recharge + ? WHERE phone = ?",
            [bonus1, bonus1, bonus1, refferaluser[0].phone],
          );
      ...
      } else if (Firstrecharge.length === 1) {
        bonus = (info[0].money * parseFloat(2.5)) / 100;
      ...
      } else if (Firstrecharge.length === 2) {
        bonus = (info[0].money * parseFloat(1.5)) / 100;
  ...
  try {
      ...
      for (let i = 0; i < f2s.length; i++) {
        ...
        for (let i = 0; i < f3s.length; i++) {
          ...
          for (let i = 0; i < f4s.length; i++) {
            ...
            const f4_time = f4s[i].time; // time f4
            let check_f4 =
              timerJoinYes(f4_time) == timerJoinYes() ? true : false;
            if (check_f4) f_all_today += 1;
            // tổng f3 mời đc hôm nay
          }
        }
    ...
    for (let i = 0; i < list_mem.length; i++) {
      ...
      for (let i = 0; i < recharge_today.length; i++) {
        ...
        let dateOnly = new Date(recharge_today[i].today)
          .toISOString()
          .split("T")[0];
        if (dateOnly == today) {
      ...
      for (let i = 0; i < withdraw_today.length; i++) {
        let dateOnly = new Date(withdraw_today[i].today)
          .toISOString()
          .split("T")[0];
        if (dateOnly == today) {
    ...
    let list_mems = [];
    async function getTodayMembersByLevel(
      connection,
      phone,
      level = 1,
      maxLevel = 6,
      ...
      for (let i = 0; i < list_mem_today.length; i++) {
        ...
        await getTodayMembersByLevel(
          connection,
          list_mem_today[i].phone,
          level + 1,
          maxLevel,
        );
      }
    ...
    for (let i = 0; i < list_mem.length; i++) {
      ...
      for (let i = 0; i < recharge_today.length; i++) {
        let time = new Date(recharge_today[i].today)
          .toISOString()
          .split("T")[0];
        if (time == today) {
      ...
      for (let i = 0; i < withdraw_today.length; i++) {
        let time = new Date(withdraw_today[i].today)
          .toISOString()
          .split("T")[0];
        if (time == today) {
    ...
    return res.status(200).json({
      ...
      todaycommission:
        teamcommissionT.length > 0 ? teamcommissionT[0].bonus : 0,
      datas: user,
      f1: f1s.length,
      f2: f2,
      f3: f3,
...
const updateBanner = async (req, res) => {
  const { ban1, ban2, ban3, ban4, ban5, ban6, ban7, ban8, ban9, ban10 } =
    req.body;
...
const activeUser = async (req, res) => {
  ...
  try {
    ...
    const [usertodays] = await connection.query(
      "SELECT COUNT(*) as total FROM users WHERE DATE(today) = ?",
      [todayDate],
    );
...
const profitLoss = async (req, res) => {
  ...
  try {
    ...
    switch (range) {
      ...
      case "thisMonth":
        dateConditionMinutes =
          "MONTH(today) = MONTH(CURDATE()) AND YEAR(today) = YEAR(CURDATE())";
        dateConditionK3 =
          "MONTH(bet_data) = MONTH(CURDATE()) AND YEAR(bet_data) = YEAR(CURDATE())";
        dateCondition5d =
          "MONTH(bet_data) = MONTH(CURDATE()) AND YEAR(bet_data) = YEAR(CURDATE())";
        break;
      ...
      case "lastMonth":
        dateConditionMinutes =
          "MONTH(today) = MONTH(CURDATE() - INTERVAL 1 MONTH) AND YEAR(today) = YEAR(CURDATE() - INTERVAL 1 MONTH)";
        dateConditionK3 =
          "MONTH(bet_data) = MONTH(CURDATE() - INTERVAL 1 MONTH) AND YEAR(bet_data) = YEAR(CURDATE() - INTERVAL 1 MONTH)";
        dateCondition5d =
          "MONTH(bet_data) = MONTH(CURDATE() - INTERVAL 1 MONTH) AND YEAR(bet_data) = YEAR(CURDATE() - INTERVAL 1 MONTH)";
        break;
...
const convertAgent = async (req, res) => {
  ...
  try {
    // Delete the record with the given ID
    await connection.query(
      "UPDATE `users` SET `level` = ? WHERE `id_user` = ?",
