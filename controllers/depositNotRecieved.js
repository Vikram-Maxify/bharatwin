...
exports.depositNotReceived = async (req, res) => {
    try {
        ...
        const saveData  =  await connection.query(` INSERT INTO DepositNotReceived 
            (UTRNumber, ReceiverUPIID, OrderNumber, OrderAmount, proof1, proof2,phone, order_id)
            VALUES (?, ?, ?, ?, ?, ?,?,?)` ,  [UTRNumber, ReceiverUPIID, OrderNumber, OrderAmount, proof1Url, proof2Url,phone,  OrderNumber])
...
exports.withdrawlProblems = async(req,res)=>{
  try{
        ...
        if (req.files) {
         ...
         await connection.query(`INSERT INTO withdrawlProblem (phone, BankName, ifsccode, AccountNumber, UsdtAddress, ProblemDescription, OrderNumber, OrderAmount, proof1, proof2, order_id)
          VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
...
exports.bankName = async (req, res) => {
  try {
     ...
     // stk-accsdt-ifsc-code
      ...
      const saveBank   =  await connection.query(`INSERT INTO user_bank 
          (stk, name_bank, phone) 
          VALUES (?, ?, ?)` , [
            BankNumber,BankName, phone 
        ...
        // Insert into the database
         ...
         await connection.query( `
          INSERT INTO USDTAddress (
              TirangaID, USDTWalletAddress, AddressScreenshot, IdentityCard, LatestDeposit ,phone
          ) VALUES (?, ?, ?, ?, ? , ?)` , [
            TirangaID, USDTWalletAddress, addressScreenshot, identityCard, latestDeposit,phone
        ])
