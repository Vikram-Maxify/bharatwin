...
const withdrawal3 = async (req, res) => {
  ...
  try {
    ...
    if (type === "BANK CARD" && money > 150000) {
      ...
      return res.status(200).json({
        message: "withdraw amount 110 to 150000",
        status: false,
        timeStamp: timeNow,
      });
    }
    ...
    if (rowss.length === 0) {
      info = JSON.stringify({ reason: "no_recharge" });
      return res.status(200).json({
...
const zilpay = async (req, res) => {
  try {
    ...
    const params = {
      amount: Number(money),
      auth: "KVYBTKJJZEBYX2BJDSN6",
      callback: "https://1xclube.com/api/webapi/zilpayCallback",
      redirect_url: "https://1xclube.com",
      user: userInfo.phone,
    };
...
const sunpaypayment = async (req, res) => {
  try {
    let auth = req.cookies.auth;
    ...
    const callbackurl = "https://1xclube.com/wallet/paynow/verify-sunpay";
    ...
    if (
      sunpayResponse === undefined ||
      sunpayResponse.tradeMsg !== "request success"
    ) {
      throw Error("Gateway er!#ror from fastpay!");
    }
    console.log("response", sunpayResponse);

    if (sunpayResponse.respCode === "success") {
      const sql = `INSERT INTO recharge SET
        id_order = ?,
        transaction_id = ?,
        phone = ?,
